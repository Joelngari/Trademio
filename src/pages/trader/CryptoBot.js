...
import { Box, AlertCircle, Loader2, Play, StopCircle, ArrowUpCircle } from 'lucide-react';
...
export default function CryptoBot() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPackages(response.data.packages.filter(p => p.type === 'crypto'));
  ...
  const handleStopSession = async () => {
    if (!activeSession?.id) return;
    try {
      await traderApi.stopSession(activeSession.id);
      setMessage({ type: 'success', text: 'Session paused. You can resume it anytime before your remaining time ends.' });
      setActiveSession((prev) => prev ? { ...prev, status: 'stopped' } : prev);
    } catch (err) {
      setMessage({ type: 'error', text: err.response?.data?.message || 'Failed to stop session' });
    }
  };
  ...
  const handleResumeSession = async () => {
    if (!activeSession?.id) return;
    try {
      await traderApi.resumeSession(activeSession.id);
      setMessage({ type: 'success', text: 'Session resumed successfully.' });
      setActiveSession((prev) => prev ? { ...prev, status: 'active' } : prev);
    } catch (err) {
      setMessage({ type: 'error', text: err.response?.data?.message || 'Failed to resume session' });
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Crypto Trading Bots</h1>
          <p className="text-gray-400">Automated volatility capture for top crypto assets.</p>
        </div>
        {activeSession && (
            ...
            {botPurchases.map((purchase) => (
              <div key={purchase.id} className="bg-black/30 border border-white/10 rounded p-4 flex items-center justify-between">
                ...
                <button
                  ...
                  className="ml-4 px-4 py-2 bg-orange-500 text-white rounded font-semibold hover:bg-orange-600 transition-all whitespace-nowrap disabled:opacity-50"
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {packages.map((pkg) => (
          <div key={pkg.id} className="bg-[#121212] border border-white/5 rounded p-8 flex flex-col relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#87ceeb]/5 rounded-full blur-3xl -mr-16 -mt-16 group-hover:bg-[#87ceeb]/10 transition-colors" />
            
            <div className="w-12 h-12 bg-orange-500/10 rounded flex items-center justify-center text-orange-500 mb-6">
              <Box size={24} />
            </div>

            <h3 className="text-xl font-bold text-white mb-2">{pkg.name}</h3>
            <p className="text-gray-400 text-sm mb-6 flex-1">
              Optimized for BTC, ETH and SOL volatility. High-frequency scalping engine.
            </p>
