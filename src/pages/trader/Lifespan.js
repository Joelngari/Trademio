  ...
  const handlePurchase = async (pkgId, isResume = false, botPurchaseId = null) => {
    ...
    try {
      ...
      const msgText = isPartial 
        ...
        : 'STK push sent! Complete the payment to activate this plan.';
            ...
            {botPurchases.map((purchase) => (
              <div key={purchase.id} className="bg-black/30 border border-white/10 rounded-xl p-4 flex items-center justify-between">
                ...
                <button
                  ...
                  className="ml-4 px-4 py-2 bg-blue-500 text-white rounded-lg font-semibold hover:bg-blue-600 transition-all whitespace-nowrap disabled:opacity-50"
                >
        ...
        {packages.map((pkg) => (
          <div key={pkg.id} className="bg-[#121212] border border-white/5 rounded-3xl p-10 flex flex-col relative overflow-hidden group">
            ...
            <div className="space-y-4 relative z-10">
              ...
              >
                {botPurchases.some(bp => bp.packageInfo?.id === pkg.id) ? 'Pending Purchase Exists' : (purchasing === pkg.id ? <Loader2 className="animate-spin" /> : activeSession ? 'Term in Progress' : 'Initiate Session')}
              </button>
